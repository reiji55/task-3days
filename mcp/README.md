...
| `set_next` | 引き継ぎの差し替え。本人が「Claude にやってほしい」と書き残した欄 |
...
ビューアの入力欄から入るのもここ。どの日付のタブにも一緒に出る。
...
| 行 | 誰のためのものか |
|---|---|
| `memo:` | 本人の覚え書き。Claude は勝手に消さない |
| `next:` | **Claude への依頼。**画面の「＋引き継ぎ」から書かれる |

`get_tasks` はどちらも返す。`next` が入っていたら拾って動き、
**済んだら何をしたかを `memo` に書いてから `next` を空にする**（`set_next` に空文字）。
`引き継ぎ:` と日本語で書かれていても同じものとして読む。

### 決めごと

- **1回の呼び出し = ファイル1つにつき1コミット。** 複数まとめられるものは配列で渡すと
  コミットも1本で済む。2つのファイルに跨るのは `set_week` の取りこぼしの引き取りだけ
...
- **中身が空になった過去の日付の見出しは、書き込みのついでに節ごと落とす**
  （消したものは `pruned` で返す）。掃除のためだけの書き込みはしない。
...
- 画面で付けた印は `status`（`done` / `miss`）。**`set_week` で写しを取り直しても
  引き継ぐ**（時刻と名前が同じものに限る）。`get_week` は `miss` の分を
  `missed` にまとめて返す
- **`missed` を見てカレンダーの予定を消すときは、必ず先に一覧を見せて確認を取る。**
  誤タップで消えると取り返しがつかない
- **入れ替えで消える枠の取りこぼしは、`tasks.txt` へ自動で移る**（下記）

毎週月曜の朝に Routine が走り、カレンダーを読んで `set_week` を呼ぶ。

#### 取りこぼしの引き取り

週が変わると先週の枠はまるごと消える。`❌` を付けたものも、印を付けないまま
時間だけ過ぎたもの（画面で光っていたもの）も一緒に流れてしまうので、
**消える前に拾い出してタスクの方へ送る。**

| 前の写しの枠 | どうなるか |
|---|---|
| `status: miss`（❌） | `tasks.txt` へ移す |
| 印なし・終わりの時刻を過ぎている | `tasks.txt` へ移す |
| `status: done`（⭕️） | 拾わない |
| まだ終わっていない | 拾わない |
| `dim` に当たる（睡眠など） | 拾わない |
| 新しい写しにも残っている | 拾わない（週の途中の取り直し） |

- 入るのは**枠があった日付のまま**。画面では `未完了` のタブに出る
- タスク名は予定の名前、時間帯は `HH:MM-HH:MM`
- 同じ日に同じ名前があれば足さない（呼び直しても二重にならない）
- **タスクを書いてから週を書き換える。** 逆にすると、タスク側が転んだときに
  取りこぼしが消えたまま拾えなくなる
- 引き取ると `tasks.txt` と `week.json` で**コミットが2本**になる
- 何を移したかは戻りの `carried` に入る

### ボード（`board.json`）

ビューアの「ガイド」（下の左のしるし）が読み書きする、**やり方の書き置き場**。
...
- カードは `id` / `title` / `created` / `goal` / `purpose` / `detail`
  （画面では `Purpose` / `Goal` / `Created on` が色の面、`Detail` が白い面に出る）
...
**ビューア側は打った瞬間に画面と `localStorage` に入れ、GitHub へ出すのは
本人が `保存` を押したとき**（1回押すと1コミット）。押さずに閉じても端末に残り、
次に開いたときに押せば上がる。
